...
compute_wilks_lambda <- function(data, fit) {
  Y <- as.matrix(data[, colnames(fit$coefficients)])
...
for (b in 1:B) {
  ...
  # same model structure but refit on bootstrap
  fit_b <- lm(
    cbind(BETAPLASMA, RETPLASMA) ~ 
      `Overall Intake` + `Diet Quality` + `Alcohol Ratio` + `Lifestyle Risk` +
      SEX + SMOKSTAT + VITUSE,
...
# bootstrap p-value. what is the proportion of bootstrap where Lambda less than or equal to observed
...
ggplot(lambda_df, aes(x = lambda)) +
  ...
  labs(
    ...
    subtitle = paste0("Red line = observed Lambda = ", round(obs_lambda, 3)),
    x = "Wilks' Lambda",
    y = "Frequency"
  ) +
  theme_minimal()
